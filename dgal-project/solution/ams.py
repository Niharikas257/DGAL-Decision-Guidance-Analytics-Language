...
def combinedSupply(input_data):
    # Extract the list of sub-services from the input data
    sub_services = input_data["services"]["combinedSupply"]["subServices"]

    # Initialize variables for total cost, constraints, and aggregated outFlow
    total_cost = 0
    #constraints_met = True
    # constraints_list = []
    #aggregated_outFlow = {}
    aggregated_outFlow = defaultdict(lambda: {"qty": 0, "item": None})
    # Initialize a dictionary to hold metrics for each sub-service
    sub_service_metrics = {}
    # Process each sub-service
    for service_name in sub_services:
        service_data = input_data["services"][service_name]
        # Initialize the sub-service metrics
        sub_service_metrics[service_name] = {
        ...
        # Process outFlow for the current sub-service
        for flow_key, flow_value in service_data["outFlow"].items():
            qty = flow_value["qty"]
            ppu = flow_value["ppu"]
            item = flow_value["item"]
            lb = flow_value.get("lb", 0)  # Default lower bound to 0 if not specified
            # Calculate the cost for this flow and add it to the sub-service and total cost
            flow_cost = qty * ppu
            sub_service_metrics[service_name]["cost"] += flow_cost
            total_cost += flow_cost
            # Update aggregated outFlow
            # if flow_key in aggregated_outFlow:
            #     aggregated_outFlow[flow_key]["qty"] += qty
            # else:
            #     aggregated_outFlow[flow_key] = {"qty": qty, "item": item}
            aggregated_outFlow[flow_key]["qty"] += qty
            if aggregated_outFlow[flow_key]["item"] is None:
                aggregated_outFlow[flow_key]["item"] = item
            # Check constraints
            # if qty < 0 or qty < lb:
            #     sub_service_metrics[service_name]["constraints"] = False
            #     constraints_met = False
            # constraints_list.append(qty >= 0)
            # constraints_list.append(qty >= lb)
            constraint_one = (qty >= 0)
            constraint_two = (qty >= lb)

        # Update the outFlow for the sub-service in metrics
        sub_service_metrics[service_name]["outFlow"] = service_data["outFlow"]

        # constraints_met = dgal.all(constraints_list)
        constraints_met = dgal.all([constraint_one, constraint_two])
    ...
    output = {
        ...
        "services": {
            "combinedSupply": {
                ...
                "debug": {
                    "flowKeys": list(aggregated_outFlow.keys()),
                    "subServiceMetrics": sub_service_metrics,
                    "subServicesFlowSupply": {k: v["qty"] for k, v in aggregated_outFlow.items()},
                    "subServicesFlowDemand": {k: 0 for k in aggregated_outFlow.keys()},
                    "inFlowKeys": [],
                    "outFlowKeys": list(aggregated_outFlow.keys()),
                    "internalFlowKeys": None,
                    "internalSupplySatisfiesDemand": True,
                    "inFlowConstraints": True,
                    "outFlowConstraints": True,
                    "subServiceConstraints": all(metric["constraints"] for metric in sub_service_metrics.values())
                }
            },
            **sub_service_metrics
        }
